...
from constants import YM_PER_PIX, XM_PER_PIX
...
def draw_back_onto_the_road(img_undistorted, Minv, line_lt, line_rt, keep_state):
    ...
    overlay_on_lane = cv2.addWeighted(img_undistorted, 0.8, road_dewarped, 0.3, 0)
    ...
    lines_mask = overlay_on_lane.copy()
    idx = np.any([line_dewarped != 0][0], axis=2)
    lines_mask[idx] = line_dewarped[idx]

    overlay_on_lane = cv2.addWeighted(src1=lines_mask, alpha=0.6, src2=overlay_on_lane, beta=0.6, gamma=0.)

    return overlay_on_lane
