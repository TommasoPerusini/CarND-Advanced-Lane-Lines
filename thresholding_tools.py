...
""" Utility class for thresholding and binarizing images"""
...
def thresh_frame_in_HLS(frame, s_thresh=(200, 255), verbose=False):
    """
    Threshold a color frame in HSV space
    """
...
def binarize(img, verbose=False):
    ...
    # saturation mask by threshold in HLS color space
    HLS_saturation_mask = thresh_frame_in_HLS(img, verbose=False)
    binary = np.logical_or(binary, HLS_saturation_mask)
